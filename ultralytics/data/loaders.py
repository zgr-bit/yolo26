...
import ultralytics.engine.results  # avoiding circular import issue with Results
...
class LoadDetections:
    """YOLOv8 detection loader.

    Wraps all dataloaders, adding the ability to load saved detections for tracking. __next__() returns list of Results,
    in addition to expected paths, imgs, info

    Path should be path to detection/track output (with confidence scores) as generated by model.predict() path/
        {source_name}_{frame_or_img_number}.txt
        ...

    Attributes:
        detections (dict): Dict of mapping frame number to .txt file containing saved detections {frame_num: file_path}
        data_loader (LoadTensor | LoadStreams | LoadScreenshots | LoadImagesAndVideos | LoadPilAndNumpy): Initialized
            Data loader
        source_type (str): Source type of data_loader
        mode (str): Current mode of data_loader
        ...
        classes (list): List of included classes of dataset. Used to enable functionality of "classes" arg for offline
            tracking
    """
    ...
    def __init__(self, path, data_loader, names, classes):
        ...
        if not os.path.isdir(path):  # check path is dir
        ...
        for f in os.listdir(path):
            if re.match(valid_file_pattern, f):
                ...
                files_dict[frame] = os.path.join(path, f)
            else:
                raise FileExistsError(f"Found file {f} in saved detections {path} that does not end in '.txt.'")

        self.detections = files_dict
        self.data_loader = data_loader
        self.source_type = data_loader.source_type
        self.mode = data_loader.mode
        self.bs = data_loader.bs
    ...
    def __next__(self):
        ...
        for frame_num, path, img in zip(list(range(self.curr_frame, self.curr_frame + num_frames)), paths, imgs):
            ...
            if frame_num in self.detections:
                ...
                # only include classes of interest if arg is given
                if self.classes is not None:
                    boxes = boxes[np.isin(boxes[:, -1], self.classes)]
                boxes = torch.tensor(boxes)
            else:  # no detections
                boxes = torch.empty(0, 6)

            res = ultralytics.engine.results.Results(img, path, self.names)
            res.update(boxes=boxes)  # issue with .unique when using numpy
            results.append(res)

        self.curr_frame += num_frames
        return paths, imgs, info, results

    def __len__(self):
        """Returns the number of batches in the object."""
        return self.data_loader.__len__()  # get len from dataloader
